...
as $$
  select coalesce(
    ...
    (select u.school_id from public.users u where u.auth_uid = auth.uid()::text limit 1)
  );
$$;
...
as $$
  select coalesce(
    ...
    (select u.role from public.users u where u.auth_uid = auth.uid()::text limit 1), ''
  );
$$;

create or replace function public.messaging_is_global_admin()
returns boolean language sql stable security definer set search_path = public
as $$ select auth.uid() is not null and public.messaging_role() in ('developer','superadmin','admin'); $$;

create or replace function public.messaging_is_school_admin(p_school_id text)
returns boolean language sql stable security definer set search_path = public
as $$ select public.messaging_is_global_admin() or (auth.uid() is not null and public.messaging_school_id() = p_school_id and public.messaging_role() in ('chairman','principal','admin')); $$;
...
create policy direct_messages_select on public.direct_messages for select using (
  public.messaging_is_global_admin() or (
    ...
    (sender_id = auth.uid()::text or receiver_id = auth.uid()::text or public.messaging_is_school_admin(school_id))
  )
);
create policy direct_messages_insert on public.direct_messages for insert with check (
  public.messaging_is_global_admin() or (
    school_id = public.messaging_school_id() and sender_id = auth.uid()::text
  )
);
create policy direct_messages_update on public.direct_messages for update using (
  public.messaging_is_global_admin() or (school_id = public.messaging_school_id() and (sender_id = auth.uid()::text or receiver_id = auth.uid()::text))
) with check (school_id = public.messaging_school_id() or public.messaging_is_global_admin());
create policy direct_messages_delete on public.direct_messages for delete using (public.messaging_is_global_admin() or (school_id = public.messaging_school_id() and sender_id = auth.uid()::text));
...
create policy school_communications_insert on public.school_communications for insert with check (
  (public.messaging_is_global_admin() or (school_id = public.messaging_school_id() and sender_id = auth.uid()::text))
);
